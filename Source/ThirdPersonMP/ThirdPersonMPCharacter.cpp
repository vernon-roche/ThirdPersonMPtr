...
void AThirdPersonMPCharacter::Tick(float delta)
{
	Super::Tick(delta);

	GEngine->AddOnScreenDebugMessage(-1, delta, FColor::Blue, FString::FromInt(CurrentHealth));
}
...
void AThirdPersonMPCharacter::OnHealthUpdate()
{
	...
	if (IsLocallyControlled())
	{
		...
		AThirdPersonMPPlayerController* pc = Cast<AThirdPersonMPPlayerController>(Controller);
		if (pc)
		{
			pc->RefreshWidget();
		}
	...
	if (Role == ROLE_Authority)
	{
		...
		if (CurrentHealth <= 0)
		{
			AThirdPersonMPGameMode* GM = Cast<AThirdPersonMPGameMode>(UGameplayStatics::GetGameMode(GetWorld()));
			//if (GM)
			{
				GM->SetPlayrToRespawn(Controller);
			}
